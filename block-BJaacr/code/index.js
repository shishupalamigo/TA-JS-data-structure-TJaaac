...
let numberOfPeople = {};
for (let house of got.houses) {
  numberOfPeople[house.name] = house.people.length; 
}
...
for (let house of got.houses) {
  for (let people of house.people) {
    console.log(people.name, ": People of GOT");
  }
}
...
console.log(`People Names : ${allPeople},  Length: ${allPeople.length}`);
...
console.log(`People Descriptions : ${allDescription},  Length: ${allDescription.length}`);
