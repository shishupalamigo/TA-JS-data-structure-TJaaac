...
console.log(movies[0].genres[movies[0].genres.length - 1], ": Last genres of 1st Movie");

// 17. Log the first genres of the second movie (using the length property of array)

console.log(movies[1].genres[movies[1].genres.length - 3], ": 1st genres of 2nd Movie");
...
for (let movie of movies) {
  console.log(movie.year > 2017, "Year of movie is greater than 2017");
}

// 23. Log the title of all three movies one by one.

for (let movie of movies) {
  console.log(movie.title, ": Title of the movie");
}
...
for (let movie of movies) {
  console.log(movie.actors.length, ": Number of Actors in the movie");
}

// 26. Log the number of genres in all three movies one by one

for (let movie of movies) {
  console.log(movie.genres.length, ": Number of Genres in the movie");
}
...
for (let movie of movies) {
  if (movie.actors.length > 1) {
    console.log(movie.title, ": Movie with more than 1 actors");
  }
}
...
for (let movie of movies) {
  if (movie.actors.length === 3) {
    console.log(movie.title, ": Movie with exactly 3 actors");
  }
}
